...
function MedicationEdit() {
    ...
    // 수정용 데이터 불러오기 (더미데이터 넣어서...)
    useEffect(() => {
        // api 받으면 fetch/axios로 데이터를 불러오기
        const dummyData = {
            drugType: '메트포르민',
            drugName: '당뇨약',
            drugDose: '1회 2정',
            ...
            drugAlarm: true,
            drugCount: 3,
            alarmTimes: ['08:00', '13:00', '18:00'],
        };

        setDrugType(dummyData.drugType);
        setDrugName(dummyData.drugName);
        setDrugDose(dummyData.drugDose);
        setDrugTime(dummyData.drugTime);
        setDrugAlarm(dummyData.drugAlarm ? '네' : '아니오');
        setDrugCount(dummyData.drugCount);
        setAlarmTimes(dummyData.alarmTimes);
    }, [id])
